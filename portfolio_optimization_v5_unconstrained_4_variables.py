...
FIGURES_PATH = '/figures/4_assets'
RESULTS_PATH = '/results/4_assets'
...
def get_train_data(period, alpha_JV_unc, value_prime_func, number_epochs, alpha, optimizer):
    ...
    print(f'Mean abs diff (ppts): {100*np.mean(np.abs(alpha_neuralnet-alpha_JV),axis = 0)}, Max alpha difference (ppts): {100*np.max(np.abs(alpha_neuralnet-alpha_JV),axis = 0)}, Mean diff (ppts): {100*np.mean(alpha_neuralnet-alpha_JV,axis = 0)}, Loss = {loss[-1]}, Total mean abs error: {100*np.mean(np.abs(alpha_neuralnet-alpha_JV))}')
